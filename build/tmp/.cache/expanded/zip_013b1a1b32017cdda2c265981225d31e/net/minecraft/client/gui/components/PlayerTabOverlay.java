...
public class PlayerTabOverlay {
   ...
   private static final ResourceLocation f_279565_ = new ResourceLocation("textures/gui/icons.png");
      ...
      for(int i4 = 0; i4 < i3; ++i4) {
         ...
         if (i4 < list.size()) {
            ...
            if (flag) {
               ...
               PlayerFaceRenderer.draw(pGuiGraphics, playerinfo1.m_105337_(), k2, l2, 8, flag2, flag1);
               k2 += 9;
            }
   ...
   protected void renderPingIcon(GuiGraphics pGuiGraphics, int p_281809_, int p_282801_, int pY, PlayerInfo pPlayerInfo) {
      ...
      pGuiGraphics.blit(f_279565_, p_282801_ + p_281809_ - 11, pY, 0, 176 + j * 8, 10, 8);
   ...
   private void renderTablistHearts(int pY, int p_283173_, int p_282149_, UUID pPlayerUuid, GuiGraphics pGuiGraphics, int pHealth) {
      ...
      if (i > 0) {
         ...
         if (k <= 3) {
            ...
            for(int l = i; l < j; ++l) {
               pGuiGraphics.blit(f_279565_, p_283173_ + l * k, pY, flag ? 25 : 16, 0, 9, 9);
            }

            for(int j1 = 0; j1 < i; ++j1) {
               pGuiGraphics.blit(f_279565_, p_283173_ + j1 * k, pY, flag ? 25 : 16, 0, 9, 9);
               if (flag) {
                  if (j1 * 2 + 1 < playertaboverlay$healthstate.displayedValue()) {
                     pGuiGraphics.blit(f_279565_, p_283173_ + j1 * k, pY, 70, 0, 9, 9);
                  }

                  if (j1 * 2 + 1 == playertaboverlay$healthstate.displayedValue()) {
                     pGuiGraphics.blit(f_279565_, p_283173_ + j1 * k, pY, 79, 0, 9, 9);
                  }
               }

               if (j1 * 2 + 1 < pHealth) {
                  pGuiGraphics.blit(f_279565_, p_283173_ + j1 * k, pY, j1 >= 10 ? 160 : 52, 0, 9, 9);
               }

               if (j1 * 2 + 1 == pHealth) {
                  pGuiGraphics.blit(f_279565_, p_283173_ + j1 * k, pY, j1 >= 10 ? 169 : 61, 0, 9, 9);
               }
            }
