...
public class InventoryScreen extends EffectRenderingInventoryScreen<InventoryMenu> implements RecipeUpdateListener {
   ...
   protected void init() {
      if (this.minecraft.gameMode.hasInfiniteItems()) {
         ...
         this.addRenderableWidget(new ImageButton(this.leftPos + 104, this.height / 2 - 22, 20, 18, 0, 0, 19, f_98830_, (p_289631_) -> {
   ...
   public static void renderEntityInInventoryFollowsMouse(GuiGraphics pGuiGraphics, int pX1, int pY1, int pX2, float pYOffset, float pMouseX, LivingEntity pEntity) {
      float f = (float)Math.atan((double)(pYOffset / 40.0F));
      float f1 = (float)Math.atan((double)(pMouseX / 40.0F));
      // Forge: Allow passing in direct angle components instead of mouse position
      renderEntityInInventoryFollowsAngle(pGuiGraphics, pX1, pY1, pX2, f, f1, pEntity);
   }

   public static void renderEntityInInventoryFollowsAngle(GuiGraphics pGuiGraphics, int pX1, int pY1, int pX2, float angleXComponent, float angleYComponent, LivingEntity pEntity) {
      ...
      renderEntityInInventory(pGuiGraphics, pX1, pY1, pX2, quaternionf, quaternionf1, pEntity);
      pEntity.yBodyRot = f2;
      pEntity.setYRot(f3);
      pEntity.setXRot(f4);
   ...
   public static void renderEntityInInventory(GuiGraphics pGuiGraphics, int pScale, int p_283401_, int p_281360_, Quaternionf pPose, @Nullable Quaternionf pCameraOrientation, LivingEntity pEntity) {
      pGuiGraphics.pose().pushPose();
      pGuiGraphics.pose().translate((double)pScale, (double)p_283401_, 50.0D);
      pGuiGraphics.pose().mulPoseMatrix((new Matrix4f()).scaling((float)p_281360_, (float)p_281360_, (float)(-p_281360_)));
