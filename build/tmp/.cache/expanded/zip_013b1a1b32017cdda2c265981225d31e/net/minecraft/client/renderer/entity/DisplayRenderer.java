...
public abstract class DisplayRenderer<T extends Display, S> extends EntityRenderer<T> {
   ...
   private Quaternionf calculateOrientation(Display.RenderState pRenderState, T pEntity) {
      Camera camera = this.entityRenderDispatcher.camera;
      Quaternionf quaternionf;
      switch (pRenderState.billboardConstraints()) {
         case FIXED:
            quaternionf = pEntity.m_269190_();
            break;
         case HORIZONTAL:
            quaternionf = (new Quaternionf()).rotationYXZ(-0.017453292F * pEntity.getYRot(), -0.017453292F * camera.getXRot(), 0.0F);
            break;
         case VERTICAL:
            quaternionf = (new Quaternionf()).rotationYXZ((float)Math.PI - ((float)Math.PI / 180F) * camera.getYRot(), ((float)Math.PI / 180F) * pEntity.getXRot(), 0.0F);
            break;
         case CENTER:
