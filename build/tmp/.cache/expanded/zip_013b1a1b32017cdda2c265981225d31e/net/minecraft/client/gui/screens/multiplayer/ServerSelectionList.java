...
public class ServerSelectionList extends ObjectSelectionList<ServerSelectionList.Entry> {
   ...
   public class OnlineServerEntry extends ServerSelectionList.Entry {
      ...
      public void render(GuiGraphics pGuiGraphics, int pIndex, int pTop, int pLeft, int pWidth, int pHeight, int pMouseX, int pMouseY, boolean pHovering, float pPartialTick) {
         ...
         pGuiGraphics.blit(ServerSelectionList.f_279631_, pLeft + pWidth - 15, pTop, (float)(k * 10), (float)(176 + l * 8), 10, 8, 256, 256);
         ...
         if (this.minecraft.options.touchscreen().get() || pHovering) {
            ...
            if (this.canJoin()) {
               if (k1 < 32 && k1 > 16) {
                  pGuiGraphics.blit(ServerSelectionList.f_99759_, pLeft, pTop, 0.0F, 32.0F, 32, 32, 256, 256);
               } else {
                  pGuiGraphics.blit(ServerSelectionList.f_99759_, pLeft, pTop, 0.0F, 0.0F, 32, 32, 256, 256);
               }
            }

            if (pIndex > 0) {
               if (k1 < 16 && l1 < 16) {
                  pGuiGraphics.blit(ServerSelectionList.f_99759_, pLeft, pTop, 96.0F, 32.0F, 32, 32, 256, 256);
               } else {
                  pGuiGraphics.blit(ServerSelectionList.f_99759_, pLeft, pTop, 96.0F, 0.0F, 32, 32, 256, 256);
               }
            }

            if (pIndex < this.screen.getServers().size() - 1) {
               if (k1 < 16 && l1 > 16) {
                  pGuiGraphics.blit(ServerSelectionList.f_99759_, pLeft, pTop, 64.0F, 32.0F, 32, 32, 256, 256);
               } else {
                  pGuiGraphics.blit(ServerSelectionList.f_99759_, pLeft, pTop, 64.0F, 0.0F, 32, 32, 256, 256);
               }
            }
         }
