...
public class GameEventListenerRenderer implements DebugRenderer.SimpleDebugRenderer {
   ...
   public void render(PoseStack pPoseStack, MultiBufferSource pBuffer, double pCamX, double pCamY, double pCamZ) {
      ...
      if (level == null) {
         ...
         for(GameEventListenerRenderer.TrackedGameEvent gameeventlistenerrenderer$trackedgameevent : this.trackedGameEvents) {
            ...
            DebugRenderer.renderFloatingText(pPoseStack, pBuffer, gameeventlistenerrenderer$trackedgameevent.gameEvent.m_157821_(), vec31.x, vec31.y + (double)0.85F, vec31.z, -7564911, 0.0075F);
         }
   ...
   public void trackGameEvent(GameEvent p_234514_, Vec3 pPosition) {
      this.trackedGameEvents.add(new GameEventListenerRenderer.TrackedGameEvent(Util.getMillis(), p_234514_, pPosition));
   }
