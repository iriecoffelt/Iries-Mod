...
public class Gui {
   ...
   public Gui(Minecraft pMinecraft, ItemRenderer pItemRenderer) {
      ...
      this.f_92995_ = new DebugScreenOverlay(pMinecraft);
   ...
   public void render(GuiGraphics pGuiGraphics, float pPartialTick) {
      ...
      if (this.minecraft.options.f_92063_) {
         this.f_92995_.render(pGuiGraphics);
      }
   ...
   public void renderCrosshair(GuiGraphics pGuiGraphics) {
      ...
      if (options.getCameraType().isFirstPerson()) {
         if (this.minecraft.gameMode.getPlayerMode() != GameType.SPECTATOR || this.canRenderCrosshairForSpectator(this.minecraft.hitResult)) {
            if (options.f_92063_ && !options.hideGui && !this.minecraft.player.isReducedDebugInfo() && !options.reducedDebugInfo().get()) {
               ...
               int i = 15;
               pGuiGraphics.blit(f_279580_, (this.screenWidth - 15) / 2, (this.screenHeight - 15) / 2, 0, 0, 15, 15);
               if (this.minecraft.options.attackIndicator().get() == AttackIndicatorStatus.CROSSHAIR) {
                  ...
                  if (flag) {
                     pGuiGraphics.blit(f_279580_, k, j, 68, 94, 16, 16);
                  } else if (f < 1.0F) {
                     int l = (int)(f * 17.0F);
                     pGuiGraphics.blit(f_279580_, k, j, 36, 94, 16, 4);
                     pGuiGraphics.blit(f_279580_, k, j, 52, 94, l, 4);
                  }
               }
   ...
   public void renderHotbar(float pPartialTick, GuiGraphics pGuiGraphics) {
      ...
      if (player != null) {
         ...
         pGuiGraphics.blit(f_92982_, i - 91, this.screenHeight - 22, 0, 0, 182, 22);
         pGuiGraphics.blit(f_92982_, i - 91 - 1 + player.getInventory().selected * 20, this.screenHeight - 22 - 1, 0, 22, 24, 22);
         if (!itemstack.isEmpty()) {
            if (humanoidarm == HumanoidArm.LEFT) {
               pGuiGraphics.blit(f_92982_, i - 91 - 29, this.screenHeight - 23, 24, 22, 29, 24);
            } else {
               pGuiGraphics.blit(f_92982_, i + 91, this.screenHeight - 23, 53, 22, 29, 24);
            }
         }
         ...
         if (this.minecraft.options.attackIndicator().get() == AttackIndicatorStatus.HOTBAR) {
            ...
            if (f < 1.0F) {
               ...
               int l1 = (int)(f * 19.0F);
               pGuiGraphics.blit(f_279580_, k2, j2, 0, 94, 18, 18);
               pGuiGraphics.blit(f_279580_, k2, j2 + 18 - l1, 18, 112 - l1, 18, l1);
            }
         }
   ...
   public void renderJumpMeter(PlayerRideableJumping pRideable, GuiGraphics pGuiGraphics, int pX) {
      ...
      pGuiGraphics.blit(f_279580_, pX, k, 0, 84, 182, 5);
      if (pRideable.getJumpCooldown() > 0) {
         pGuiGraphics.blit(f_279580_, pX, k, 0, 74, 182, 5);
      } else if (j > 0) {
         pGuiGraphics.blit(f_279580_, pX, k, 0, 89, j, 5);
      }
   ...
   public void renderExperienceBar(GuiGraphics pGuiGraphics, int pX) {
      ...
      if (i > 0) {
         ...
         pGuiGraphics.blit(f_279580_, pX, l, 0, 64, 182, 5);
         if (k > 0) {
            pGuiGraphics.blit(f_279580_, pX, l, 0, 69, k, 5);
         }
      }
      ...
      if (player != null) {
         ...
         for(int k3 = 0; k3 < 10; ++k3) {
            if (i3 > 0) {
               int l3 = i1 + k3 * 8;
               if (k3 * 2 + 1 < i3) {
                  pGuiGraphics.blit(f_279580_, l3, k2, 34, 9, 9, 9);
               }

               if (k3 * 2 + 1 == i3) {
                  pGuiGraphics.blit(f_279580_, l3, k2, 25, 9, 9, 9);
               }

               if (k3 * 2 + 1 > i3) {
                  pGuiGraphics.blit(f_279580_, l3, k2, 16, 9, 9, 9);
               }
            }
         }
         ...
         if (k5 == 0) {
            ...
            for(int i4 = 0; i4 < 10; ++i4) {
               ...
               int i5 = j1 - i4 * 8 - 9;
               pGuiGraphics.blit(f_279580_, i5, j4, 16 + l4 * 9, 27, 9, 9);
               if (i4 * 2 + 1 < l) {
                  pGuiGraphics.blit(f_279580_, i5, j4, k4 + 36, 27, 9, 9);
               }

               if (i4 * 2 + 1 == l) {
                  pGuiGraphics.blit(f_279580_, i5, j4, k4 + 45, 27, 9, 9);
               }
            }
         ...
         if (player.isEyeInFluid(FluidTags.WATER) || i6 < l5) {
            ...
            for(int j5 = 0; j5 < k6 + l6; ++j5) {
               if (j5 < k6) {
                  pGuiGraphics.blit(f_279580_, j1 - j5 * 8 - 9, l2, 16, 18, 9, 9);
               } else {
                  pGuiGraphics.blit(f_279580_, j1 - j5 * 8 - 9, l2, 25, 18, 9, 9);
               }
            }
         }
   ...
   private void renderHeart(GuiGraphics pGuiGraphics, Gui.HeartType pHeartType, int pX, int pY, int p_283188_, boolean pHardcore, boolean pHalfHeart) {
      pGuiGraphics.blit(f_279580_, pX, pY, pHeartType.m_168734_(pHalfHeart, pHardcore), p_283188_, 9, 9);
   }
         ...
         if (i != 0) {
            ...
            for(boolean flag = false; i > 0; j1 += 20) {
               ...
               for(int l1 = 0; l1 < k1; ++l1) {
                  ...
                  int j2 = 0;
                  int k2 = l - l1 * 8 - 9;
                  pGuiGraphics.blit(f_279580_, k2, i1, 52 + j2 * 9, 9, 9, 9);
                  if (l1 * 2 + 1 + j1 < j) {
                     pGuiGraphics.blit(f_279580_, k2, i1, 88, 9, 9, 9);
                  }

                  if (l1 * 2 + 1 + j1 == j) {
                     pGuiGraphics.blit(f_279580_, k2, i1, 97, 9, 9, 9);
                  }
               }
   ...
   public void onDisconnected() {
      ...
      this.minecraft.options.f_92063_ = false;
