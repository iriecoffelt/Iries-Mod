      ...
      public void render(GuiGraphics pGuiGraphics, int pIndex, int pTop, int pLeft, int pWidth, int pHeight, int pMouseX, int pMouseY, boolean pHovering, float pPartialTick) {
         ...
         if (this.showHoverOverlay() && (this.minecraft.options.touchscreen().get() || pHovering || this.parent.getSelected() == this && this.parent.isFocused())) {
            ...
            if (this.pack.canSelect()) {
               if (i < 32) {
                  pGuiGraphics.blit(TransferableSelectionList.f_100052_, pLeft, pTop, 0.0F, 32.0F, 32, 32, 256, 256);
               } else {
                  pGuiGraphics.blit(TransferableSelectionList.f_100052_, pLeft, pTop, 0.0F, 0.0F, 32, 32, 256, 256);
               }
            } else {
               if (this.pack.canUnselect()) {
                  if (i < 16) {
                     pGuiGraphics.blit(TransferableSelectionList.f_100052_, pLeft, pTop, 32.0F, 32.0F, 32, 32, 256, 256);
                  } else {
                     pGuiGraphics.blit(TransferableSelectionList.f_100052_, pLeft, pTop, 32.0F, 0.0F, 32, 32, 256, 256);
                  }
               }

               if (this.pack.canMoveUp()) {
                  if (i < 32 && i > 16 && j < 16) {
                     pGuiGraphics.blit(TransferableSelectionList.f_100052_, pLeft, pTop, 96.0F, 32.0F, 32, 32, 256, 256);
                  } else {
                     pGuiGraphics.blit(TransferableSelectionList.f_100052_, pLeft, pTop, 96.0F, 0.0F, 32, 32, 256, 256);
                  }
               }

               if (this.pack.canMoveDown()) {
                  if (i < 32 && i > 16 && j > 16) {
                     pGuiGraphics.blit(TransferableSelectionList.f_100052_, pLeft, pTop, 64.0F, 32.0F, 32, 32, 256, 256);
                  } else {
                     pGuiGraphics.blit(TransferableSelectionList.f_100052_, pLeft, pTop, 64.0F, 0.0F, 32, 32, 256, 256);
                  }
               }
            }
         }
