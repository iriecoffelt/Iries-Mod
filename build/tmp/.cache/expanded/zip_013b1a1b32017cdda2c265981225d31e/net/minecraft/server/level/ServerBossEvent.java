...
public class ServerBossEvent extends BossEvent {
   ...
   private void broadcast(Function<BossEvent, ClientboundBossEventPacket> pPacketGetter) {
      if (this.visible) {
         ...
         for(ServerPlayer serverplayer : this.players) {
            serverplayer.connection.m_9829_(clientboundbosseventpacket);
         }
      }
   ...
   public void addPlayer(ServerPlayer pPlayer) {
      if (this.players.add(pPlayer) && this.visible) {
         pPlayer.connection.m_9829_(ClientboundBossEventPacket.createAddPacket(this));
      }
   ...
   public void removePlayer(ServerPlayer pPlayer) {
      if (this.players.remove(pPlayer) && this.visible) {
         pPlayer.connection.m_9829_(ClientboundBossEventPacket.createRemovePacket(this.getId()));
      }
   ...
   public void setVisible(boolean pVisible) {
      if (pVisible != this.visible) {
         ...
         for(ServerPlayer serverplayer : this.players) {
            serverplayer.connection.m_9829_(pVisible ? ClientboundBossEventPacket.createAddPacket(this) : ClientboundBossEventPacket.createRemovePacket(this.getId()));
         }
      }
