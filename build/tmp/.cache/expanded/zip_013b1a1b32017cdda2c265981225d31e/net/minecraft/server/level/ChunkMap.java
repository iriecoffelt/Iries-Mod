...
public class ChunkMap extends ChunkStorage implements ChunkHolder.PlayerProvider {
   ...
   protected void m_140167_(int p_140168_) {
      ...
      if (i != this.f_140126_) {
         ...
         this.f_140126_ = i;
         this.distanceManager.updatePlayerTickets(this.f_140126_);
         ...
         for(ChunkHolder chunkholder : this.updatingChunkMap.values()) {
            ...
            this.getPlayers(chunkpos, false).forEach((p_214864_) -> {
               ...
               boolean flag1 = m_200878_(chunkpos.x, chunkpos.z, sectionpos.x(), sectionpos.z(), this.f_140126_);
   ...
   protected void updateChunkTracking(ServerPlayer pPlayer, ChunkPos p_183756_, MutableObject<ClientboundLevelChunkWithLightPacket> p_183757_, boolean p_183758_, boolean p_183759_) {
      if (pPlayer.level() == this.level) {
         if (p_183759_ && !p_183758_) {
            ...
            if (chunkholder != null) {
               ...
               if (levelchunk != null) {
                  this.m_183760_(pPlayer, p_183757_, levelchunk);
               }
         ...
         if (!p_183759_ && p_183758_) {
            pPlayer.m_9088_(p_183756_);
            net.minecraftforge.event.ForgeEventFactory.fireChunkUnWatch(pPlayer, p_183756_, this.level);
         }
   ...
   boolean anyPlayerCloseEnoughForSpawning(ChunkPos pChunkPos) {
      ...
      if (!this.distanceManager.hasPlayersNearby(i)) {
         ...
         for(ServerPlayer serverplayer : this.playerMap.getAllPlayers(i)) {
   ...
   private SectionPos updatePlayerPos(ServerPlayer pPlayer) {
      ...
      pPlayer.connection.m_9829_(new ClientboundSetChunkCacheCenterPacket(sectionpos.x(), sectionpos.z()));
      return sectionpos;
   }
   ...
   public void move(ServerPlayer pPlayer) {
      ...
      if (flag2 || flag != flag1) {
         ...
         if (i != j) {
            this.playerMap.m_8245_(i, j, pPlayer);
         }
      }
   ...
   public List<ServerPlayer> getPlayers(ChunkPos pPos, boolean pBoundaryOnly) {
      Set<ServerPlayer> set = this.playerMap.getAllPlayers(pPos.toLong());
      ImmutableList.Builder<ServerPlayer> builder = ImmutableList.builder();

      for(ServerPlayer serverplayer : set) {
         SectionPos sectionpos = serverplayer.getLastSectionPos();
         if (pBoundaryOnly && m_183828_(pPos.x, pPos.z, sectionpos.x(), sectionpos.z(), this.f_140126_) || !pBoundaryOnly && m_200878_(pPos.x, pPos.z, sectionpos.x(), sectionpos.z(), this.f_140126_)) {
            builder.add(serverplayer);
         }
      }
   ...
   public void resendBiomesForChunks(List<ChunkAccess> pChunks) {
      ...
      map.forEach((p_274835_, p_274836_) -> {
         p_274835_.connection.m_9829_(ClientboundChunksBiomesPacket.forChunks(p_274836_));
      });
   }
   ...
   private void m_183760_(ServerPlayer p_183761_, MutableObject<ClientboundLevelChunkWithLightPacket> p_183762_, LevelChunk p_183763_) {
      ...
      p_183761_.m_184135_(p_183763_.getPos(), p_183762_.getValue());
      ...
      if (!list.isEmpty()) {
         for(Entity entity1 : list) {
            p_183761_.connection.m_9829_(new ClientboundSetEntityLinkPacket(entity1, ((Mob)entity1).getLeashHolder()));
         }
      }

      if (!list1.isEmpty()) {
         for(Entity entity2 : list1) {
            p_183761_.connection.m_9829_(new ClientboundSetPassengersPacket(entity2));
         }
      }
   ...
   class TrackedEntity {
      ...
      public void broadcastAndSend(Packet<?> pPacket) {
         ...
         if (this.entity instanceof ServerPlayer) {
            ((ServerPlayer)this.entity).connection.m_9829_(pPacket);
         }
      ...
      public void updatePlayer(ServerPlayer pPlayer) {
         if (pPlayer != this.entity) {
            ...
            double d0 = (double)Math.min(this.getEffectiveRange(), ChunkMap.this.f_140126_ * 16);
