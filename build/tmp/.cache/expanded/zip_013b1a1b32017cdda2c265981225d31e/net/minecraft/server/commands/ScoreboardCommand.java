...
public class ScoreboardCommand {
   ...
   private static int clearDisplaySlot(CommandSourceStack pSource, int p_138479_) throws CommandSyntaxException {
      Scoreboard scoreboard = pSource.getServer().getScoreboard();
      if (scoreboard.getDisplayObjective(p_138479_) == null) {
         ...
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.objectives.display.cleared", Scoreboard.m_83494_()[p_138479_]);
         }, true);
         return 0;
      }
   }

   private static int setDisplaySlot(CommandSourceStack pSource, int p_138482_, Objective pObjective) throws CommandSyntaxException {
      Scoreboard scoreboard = pSource.getServer().getScoreboard();
      if (scoreboard.getDisplayObjective(p_138482_) == pObjective) {
         throw ERROR_DISPLAY_SLOT_ALREADY_SET.create();
      } else {
         scoreboard.setDisplayObjective(p_138482_, pObjective);
         pSource.sendSuccess(() -> {
            return Component.translatable("commands.scoreboard.objectives.display.set", Scoreboard.m_83494_()[p_138482_], pObjective.getDisplayName());
         }, true);
         return 0;
      }
   }
