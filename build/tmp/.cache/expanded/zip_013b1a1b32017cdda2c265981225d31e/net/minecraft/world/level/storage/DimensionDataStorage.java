...
public class DimensionDataStorage {
   ...
   public <T extends SavedData> T computeIfAbsent(Function<CompoundTag, T> p_164862_, Supplier<T> p_164863_, String pName) {
      T t = this.get(p_164862_, pName);
      if (t != null) {
         return t;
      } else {
         T t1 = p_164863_.get();
   ...
   @Nullable
   public <T extends SavedData> T get(Function<CompoundTag, T> p_164859_, String pName) {
      ...
      if (saveddata == null && !this.cache.containsKey(pName)) {
         saveddata = this.readSavedData(p_164859_, pName);
   ...
   @Nullable
   private <T extends SavedData> T readSavedData(Function<CompoundTag, T> pDeserializer, String pName) {
      try {
   ...
   public CompoundTag readTagFromDisk(String pFilename, int pVersion) throws IOException {
      ...
      try (
         ...
         compoundtag1 = DataFixTypes.SAVED_DATA.update(this.fixerUpper, compoundtag, i, pVersion);
      }
