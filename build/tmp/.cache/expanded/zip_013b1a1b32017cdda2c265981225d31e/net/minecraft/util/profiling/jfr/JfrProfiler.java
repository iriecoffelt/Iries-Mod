...
public class JfrProfiler implements JvmProfiler {
   ...
   public void onPacketReceived(int pPacketId, int pBytes, SocketAddress pRemoteAddress, int p_185305_) {
      if (PacketReceivedEvent.TYPE.isEnabled()) {
         (new PacketReceivedEvent(pPacketId, pBytes, pRemoteAddress, p_185305_)).commit();
      }

      if (NetworkSummaryEvent.TYPE.isEnabled()) {
         this.networkStatFor(pRemoteAddress).trackReceivedPacket(p_185305_);
      }

   }

   public void onPacketSent(int pPacketId, int pBytes, SocketAddress pRemoteAddress, int p_185326_) {
      if (PacketSentEvent.TYPE.isEnabled()) {
         (new PacketSentEvent(pPacketId, pBytes, pRemoteAddress, p_185326_)).commit();
      }

      if (NetworkSummaryEvent.TYPE.isEnabled()) {
         this.networkStatFor(pRemoteAddress).trackSentPacket(p_185326_);
      }
