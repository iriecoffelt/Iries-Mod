...
public enum ConnectionProtocol implements BundlerInfo.Provider {
   ...
   private ConnectionProtocol(int p_129580_, ConnectionProtocol.ProtocolBuilder pBuilder) {
      this.id = p_129580_;
      this.flows = pBuilder.flows;
   }

   public int m_264521_(PacketFlow p_265671_, Packet<?> p_265442_) {
      return this.flows.get(p_265671_).getId(p_265442_.getClass());
   }

   public BundlerInfo m_264121_(PacketFlow p_265790_) {
   ...
   static {
      for(ConnectionProtocol connectionprotocol : values()) {
         ...
         connectionprotocol.flows.forEach((p_264719_, p_264720_) -> {
            p_264720_.m_264538_((p_195615_) -> {
               if (f_129572_.containsKey(p_195615_) && f_129572_.get(p_195615_) != connectionprotocol) {
                  throw new IllegalStateException("Packet " + p_195615_ + " is already assigned to protocol " + f_129572_.get(p_195615_) + " - can't reassign to " + connectionprotocol);
               } else {
                  f_129572_.put(p_195615_, connectionprotocol);
               }
            });
         });
      }
